...
### Local RPM Build and Test

For testing the RPM spec locally before a Fedora review submission or while iterating on spec changes, the full build can be performed without fedpkg or dist-git access. This workflow runs entirely on a local Fedora machine.

#### Additional prerequisites

```bash
sudo dnf install go-vendor-tools rpm-build rpmlint rpmdevtools
```

#### Build steps

1. Set up the rpmbuild directory tree:

```bash
rpmdev-setuptree
```

2. Download the source tarball from GitHub (using the version in the spec):

```bash
spectool -g -R complytime-providers.spec
```

3. Generate the vendor archive from the source tarball:

```bash
go_vendor_archive create --config go-vendor-tools.toml complytime-providers.spec
```

4. Copy the vendor archive and license config to the rpmbuild sources directory:

```bash
cp complytime-providers-*-vendor.tar.bz2 ~/rpmbuild/SOURCES/
...
6. Rebuild the source RPM into binary RPMs:

```bash
rpmbuild --rebuild ~/rpmbuild/SRPMS/complytime-providers-*.src.rpm
```

7. Run rpmlint on the built packages:

```bash
rpmlint ~/rpmbuild/SRPMS/complytime-providers-*.src.rpm \
    ~/rpmbuild/RPMS/x86_64/complytime-providers-*.rpm
```

8. Optionally, test in mock for a clean buildroot (requires `mock` group membership):

```bash
mock -r fedora-rawhide-x86_64 rebuild ~/rpmbuild/SRPMS/complytime-providers-*.src.rpm
```

#### Expected rpmlint output

The following rpmlint messages are expected and can be safely ignored:

- `spelling-error` for `complyctl`, `gRPC`, `conftest` — these are actual tool/protocol names, not typos
- `no-binary` on the main `complytime-providers` package — this package holds only shared license files (by design)
- `invalid-url Source1` — the vendor archive is a local file generated by `go_vendor_archive`, not a downloadable URL
- `no-documentation` on sub-packages — documentation is in the main package
- `specfile-warning: autopatch` — harmless warning when no patches are defined
